...
def import_objs(path_in: str, path_out: str, objs_out_path: str):
	"""
	...
	"""
	grouped_objs = nicify_filenames_group_objs(path_in)
	...
	for obj_id, obj_node in grouped_objs.items():
		...
		if obj_id not in obj_offsets:
			log_info("Object \"" + obj_id + "\" offset not found in convert_data, assuming (0, 0)")
		...
		for txt_type, txt_list in obj_node.items():
			if txt_type == TXT_TYPE_MAIN:
				...
				log_info("Object \"" + obj_id + "\": unknown texture type (" + str(txt_type) + ")")
			...
			for obj_type_node in obj_node.values():
				for txt_variant in obj_type_node:
					...
					if os.path.exists(item_out_path):
						log_verbose(item_out_path + " already exists, skipping")
						continue
	...
	log_warn("Please manually fix trimmed and/or unpadded textures for: \"" + ("\", \"".join(obj_copy_blacklist) + "\"!"))
...
if __name__ == "__main__":
	...
	import_objs(args.input, args.output, args.objs)
