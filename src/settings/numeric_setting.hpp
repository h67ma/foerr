...
class NumericSetting: public Setting
{
	...
	public:
		NumericSetting(std::string key, uint defaultVal);
		NumericSetting(std::string key, uint defaultVal, const std::function<bool(uint)> constraint,
					   std::string valueHint); // both constraint and hint must be set if one of them is set
